		...
		<form:form action="${pageContext.request.contextPath}/product/add" commandName="productcommand">
			<table>
				...
				<tr>
					...
					<td><form:input path="pid" id="pid" disabled="true" readonly="true"/></td>
					</c:when>
					<c:otherwise>
				...
				<tr>
					<td colspan="2"><c:if test="${!empty productcommand.name}"><input type="submit" value="Edit" /></c:if>
					<c:if test="${empty productcommand.name}"><input type="submit" value="Submit" /></c:if>
					</td>
				</tr>
			</table>
		</form:form>
